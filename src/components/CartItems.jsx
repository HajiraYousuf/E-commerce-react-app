...
const CartItems = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row p-4 lg:p-8 space-y-6
     lg:space-y-0 lg:space-x-6 ">
    {/* cart items section */}
      <div className='flex-1 '>
        <h2 className="text-2xl font-semibold mb-4 ">{ products.length > 0 ? "your cart items" 
        : "your cart is empty right now please go a head and add some items"}
        </h2>
        {
          products.map((product) => (
            <div className="flex items-start space-x-4 mv-6 ">
              <img 
                  ...
                  <div className='flex items-center justify-between mt-2 '>
                    ...
                    <div className='flex items-center space-x-2 '>
                      ...
                      <div className='flex items-center space-x-2 '>
                        ...
                        <input 
                        type="number"
                        onChange={()=> updateProductQuantity(product,event.target.value)} 
                    ...
                    <button className='text-red-500 hover:text-red-700 transition-colors duration-200 ease-in-out'>
                      <GoTrash onClick={()=> removeFromCart(product)} className='w-6 h-6'/>
                      </button>
                  </div>
                </div>
